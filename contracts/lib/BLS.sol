...
library BLS {
    ...
    bytes32 private constant DST = keccak256("BLS_SIG_BN254G1_XMD:SHA-256_SVDW_RO_NUL_"); //0x416a79f61c64a68f9946f715ec2b7077204a431ad6c623c2b7e464d4b60b0ed6

    function blsSignsVerification(
        bytes memory signature,
        bytes memory pubkey,
        bytes memory message
    ) internal view returns (bool, bool) {
        require(signature.length == 64, "signature bytes must be 64 bytes long");
        require(pubkey.length == 128, "pubkey bytes must be 128 bytes long");
        uint256[2] memory sig;
        assembly {
            mstore(sig, mload(add(signature, 0x20)))
            mstore(add(sig, 0x20), mload(add(signature, 0x40)))
        }
        uint256[4] memory pk;
        assembly {
            mstore(pk, mload(add(pubkey, 0x20)))
            mstore(add(pk, 0x20), mload(add(pubkey, 0x40)))
            mstore(add(pk, 0x40), mload(add(pubkey, 0x60)))
            mstore(add(pk, 0x60), mload(add(pubkey, 0x80)))
        }

        uint256[2] memory messageG1 = hashToPoint(DST, message);
        return verifySingle(sig, pk, messageG1);
    }

    function blsAggSignsVerification(
        bytes memory aggsignatures,
        bytes memory pubkeys,
        bytes memory message
    ) internal view returns (bool, bool) {
        require(aggsignatures.length == 64, "signature bytes must be 64 bytes long");
        require(pubkeys.length % 128 == 0, "pubkey bytes must be multiple of 128 bytes long");
        uint256[2] memory sigs;
        assembly {
            mstore(sigs, mload(add(aggsignatures, 0x20)))
            mstore(add(sigs, 0x20), mload(add(aggsignatures, 0x40)))
        }
        uint256 size = pubkeys.length / 32;
        uint256[] memory pks = new uint256[](size);
        for (uint256 i = 0; i < size; i++) {
            assembly {
                let value := mload(add(pubkeys, add(mul(i, 32), 32)))
                mstore(add(pks, add(mul(i, 32), 32)), value)
            }
        }

        uint256[2] memory messageG1 = hashToPoint(DST, message);
        return verifyMultiple(sigs, pks, messageG1);
    }
    ...
    function verifyMultiple(
        ...
        input[5] = N_G2_Y0;
        for (uint256 i = 0; i < size; i++) {
            input[i * 6 + 6] = messages[0];
            input[i * 6 + 7] = messages[1];
            input[i * 6 + 8] = pubkeys[i * 4];
            input[i * 6 + 9] = pubkeys[i * 4 + 1];
            input[i * 6 + 10] = pubkeys[i * 4 + 2];
            input[i * 6 + 11] = pubkeys[i * 4 + 3];
        }
        uint256[1] memory out;
